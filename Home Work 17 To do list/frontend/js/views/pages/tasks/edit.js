...
class Edit extends Component {
  ...
  setActions() {
    const editTaskTitle =
        document.getElementsByClassName('task-edit__title')[0],
      editTaskDescription = document.getElementsByClassName(
        'task-edit__description'
      )[0],
      saveTaskBtn = document.getElementsByClassName('task-edit__btn-save')[0];

    editTaskTitle.addEventListener(
      'keyup',
      () => (saveTaskBtn.disabled = !editTaskTitle.value.trim())
    );
    saveTaskBtn.addEventListener('click', () =>
      this.editTask(editTaskTitle, editTaskDescription)
    );
  }
